...
resource "aws_lb_listener" "ingress_alb_listener" {
  load_balancer_arn = aws_lb.ingress_alb.arn # ##giving load balancer arn
  port              = "443"
  protocol          = "HTTPS"
  ssl_policy        = "ELBSecurityPolicy-TLS13-1-3-2021-06"
  certificate_arn   = local.ingress_alb_certificate_arn   # we will attach our certificate arn to here to ssl validation and termiantion  

  default_action {# defalut-rule
    type = "fixed-response"
...
# Till here load balancer will be created  and we added listener hit default port and http and defalut action, now it have  default dns name without our domain name like
...
#now it have session key encrypts with public key and send back to LB and LB decrypts it private key and now they have session key and request flows on  


resource "aws_route53_record" "ingress_alb" {
  zone_id = var.zone_id
  name    = "*.${var.domain_name}" # *.vigi-devops.fun
  type    = "A"
...
# if anyone hit at *.vigi-devops.fun then forwarded to my lb dns
#user → *.vigi-devops.fun
#Route53 → ALB DNS name
#ALB → Target Group → Pods
